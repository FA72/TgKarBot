...
internal class Asks
{
    ...
    public static async Task<string> CheckAsk(long userId, string message)
    {
        ...
        //    if (test.Count != 0 && CheckLastPairFull(test))
        ...
        else 
        {
            ...
            else
            {
                ...
                output.Append($"Вы можете выпить в баре (таймер на паузе). Для продолжения игры вам нужно ответить на второй вопрос из пары или ввести /next для пропуска этого вопроса (это действие запустит таймер. " +
                              $"Обратите внимание, что после ввода команды /next вы не сможете остановить время до правильного ответа на любой вопрос следующей пары).");
            }
